 ...
 */
  ...
  FunctionAvailability availability({SonyCameraDevice? device}) {
    ...
    if (this == device.api.setZoomSetting)
      return device.api.checkFunctionAvailability(ItemId.ZoomSetting, ApiMethodId.SET);

    if (this == device.api.getZoomSetting)
      return device.api.checkFunctionAvailability(ItemId.ZoomSetting, ApiMethodId.GET);

    if (this == device.api.setSilentShooting)
      return device.api.checkFunctionAvailability(ItemId.SilentShooting, ApiMethodId.SET);

    if (this == device.api.getSilentShooting)
      return device.api.checkFunctionAvailability(ItemId.SilentShooting, ApiMethodId.GET);

    if (this == device.api.setLiveViewFrameInfo)
      return device.api.checkFunctionAvailability(ItemId.LiveViewFrameInfo, ApiMethodId.SET);

    if (this == device.api.getLiveViewFrameInfo)
      return device.api.checkFunctionAvailability(ItemId.LiveViewFrameInfo, ApiMethodId.GET);
